...
def main():
	...
	parser.add_argument('--new_n_classes', type=int)
	args = parser.parse_args()
	
	# Set random seeds for reproducibility (use run number for different seeds)
	run_seed = 42 + int(args.run)
	...
	train_dataset = datasets.MNIST(dset_dir, train=True, download=True,
								   transform=transforms_to_apply, )  # this part will go to training 
	test_dataset = datasets.MNIST(dset_dir, train=False,
								  transform=transforms_to_apply, )  # this part will go to eval

	train_indices = [idx for idx, target in
					 enumerate(train_dataset.targets.detach().numpy()) if target in
					 selected_train_classes]  # take only those stimuli that have indices matching with the target
	test_indices = [idx for idx, target in enumerate(test_dataset.targets.detach().numpy())
					if target in selected_train_classes]  # same but for the test
	
	# Create label mapping for remapping original labels to [0, n_classes-1]
	label_mapping = {old_label: new_label for new_label, old_label in enumerate(selected_train_classes)}

	# Training set loader
	train_loader = torch.utils.data.DataLoader(
		RemappedDataset(train_dataset, train_indices, label_mapping),
		batch_size=args.train_batch_size, shuffle=True, **kwargs)
	# Test set loader
	test_loader = torch.utils.data.DataLoader(
		RemappedDataset(test_dataset, test_indices, label_mapping),
		batch_size=args.test_batch_size, shuffle=True, **kwargs)

	# Build model
	log.info('Building model...')
	encoder = Encoder(args).to(device)
	# Use n_classes for the first phase classifier
	class_out_10 = Class_out(args, output_dim=n_classes).to(device)
	conf_out_10 = Conf_out(args).to(device)
	all_modules_10 = nn.ModuleList([encoder, class_out_10, conf_out_10])

	# Create optimizer
	log.info('Setting up optimizer...')
	optimizer = optim.Adam(all_modules_10.parameters(), lr=args.lr)

	# Train
	log.info('Training begins...')
	for epoch in range(1, args.epochs + 1):
		# Training loop
		train(args, encoder, class_out_10, conf_out_10, device, train_loader, optimizer, epoch)

	#redo training for fixed encoder
	new_n_classes = args.new_n_classes
	#all_digit_classes = np.arange(10)  # create a vector of int (0; 9)
	#np.random.shuffle(all_digit_classes)  # random shuffle
	selected_train_classes = all_digit_classes[:new_n_classes]  # choose first n classes of
	log.info("Selected classes:")  #print this text
	log.info(str(selected_train_classes))  # print the value of selected classes (digits)
	...
	# Create label mapping for remapping original labels to [0, new_n_classes-1]
	label_mapping = {old_label: new_label for new_label, old_label in enumerate(selected_train_classes)}
	...
	# Build model
	log.info('Building fianl model...')
	old_encoder = copy.deepcopy(encoder)
	for param in encoder.parameters():
		param.requires_grad = False

	# Use new_n_classes for the second phase classifier
	...
	# Train
	log.info('Training of final model begins...')
	for epoch in range(1, args.epochs + 1):
		# Training loop
	...
	test_dir = './test/'
	check_path(test_dir)
	classes_dir = test_dir + f'classes{str(n_classes)}_{str(new_n_classes)}_v5/'
