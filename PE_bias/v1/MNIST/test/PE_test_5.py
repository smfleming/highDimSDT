...
def plot(args):
	...
	for r in range(args.N_runs):
		# Run directory - updated for v5
		run_dir = f'./classes{args.n_classes}_{args.new_n_classes}_v5/run' + str(r+1) + '/'
		# Load test results
	...
	pandas_dict = {
		...
		"low_PE_signal": np.repeat(low_PE_signal,args.N_runs),
		"high_PE_signal": np.repeat(high_PE_signal,args.N_runs),
		"low_PE_noise": np.repeat(noise_test_vals[0],args.N_runs),
		"high_PE_noise": np.repeat(noise_test_vals[1],args.N_runs),
		"dimensionality": np.repeat(args.n_classes,args.N_runs),
		"target_acc": np.repeat(target_acc,args.N_runs)
	}

	df = pd.DataFrame(data=pandas_dict)
	# Updated filename to include v5
	df.to_csv(f'./df{args.n_classes}_{args.new_n_classes}_v5_ta{int(target_acc*100)}.csv', index=False)

	# Stats
	# Open file - updated filename to include v5
	stats_fname = f'./PE_stats{args.n_classes}_{args.new_n_classes}_v5.txt'
	...
	# T-test for accuracy
	acc_t, acc_p = ttest_ind(high_PE_test_acc,low_PE_test_acc)
	...
	conf_t, conf_p = ttest_ind(high_PE_test_conf,low_PE_test_conf)
	...
	plt.yticks([0.45,0.5,0.55,0.6,0.65, 0.7, 0.75],['0.45','0.5','0.55','0.6','0.65','0.7', '0.75'], fontsize=tick_font_size)
	...
	# Save plot - updated filename to include v5
	plot_fname = f'./PE_bias_MNIST{args.n_classes}_{args.new_n_classes}_v5_ta{int(target_acc*100)}.png'
